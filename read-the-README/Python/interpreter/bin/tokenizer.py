...
class Tokenizer:
			...
			def currentNum(self):
				...
				if self.currenttoken < len(self.tokens):
					...
					if token in self.special:
						...
						return 33
				self.debug = "There was an invalid token.\nFile \'"+self.inputfile.name+"\', line "+str(self.currentline)
				return -1
		...
		specials = None
		def __init__(self, filename):
			if not Tokenizer.instance:
				Tokenizer.instance = Tokenizer.__OnlyOne(filename)
				Tokenizer.reserved = Tokenizer.instance.reserved
				Tokenizer.specials = Tokenizer.instance.special
			else:
				print("ERROR: singleton pattern violation.")
				quit()
		...
		def getToken(self):
			return Tokenizer.instance.tokens[Tokenizer.instance.currenttoken]

		def isId(self,token):
			return Tokenizer.instance.isIdentifier(token)

		def parseError(self):
			print("There was an unexpected token: "+str(Tokenizer.instance.getToken())+"\nFile \'"+Tokenizer.instance.inputfile.name+"\', line "+str(Tokenizer.instance.currentline))
			quit()
