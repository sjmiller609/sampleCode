...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        // for positive a, remainder operator is fine.
        int mod = a % b;
        // if the result is negative, simply add the clock size once.
        if (mod < 0) {
            mod += b;
        }
        return mod;
     ...
     * </pre>
     */
    // will construct hash table here
    private void createNewRep(int hashTableSize) {

        // this makes an empty Array of size @code hashTableSize
        // with entries of type Map<K,V>
        this.hashTable = new Array1L<Map<K, V>>(hashTableSize);

        // initialize all Array elements to an empty Map2
        for (int i = 0; i < hashTableSize; i++) {
            Map<K, V> temp = new Map2<>();
            this.hashTable.setEntry(i, temp);
        }
    ...
    public final void add(K key, V value) {
        ...
        Map<K, V> temp = this.hashTable.entry(bucket);
        temp.add(key, value);
        this.hashTable.setEntry(bucket, temp);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int bucket = mod(key.hashCode(), this.hashTable.length());
        Map<K, V> temp = this.hashTable.entry(bucket);
        Pair<K, V> result = temp.remove(key);
        this.hashTable.setEntry(bucket, temp);
    ...
    public final Pair<K, V> removeAny() {
        ...
        for (int i = 0; i < this.hashTable.length(); i++) {
            if (this.hashTable.mayBeExamined(i)) {
                Map<K, V> temp = this.hashTable.entry(i);
                result = temp.removeAny();
                this.hashTable.setEntry(i, temp);
    ...
    public final int size() {
        ...
        // better than adding up all the this.hashTable ones
        return this.size;
    }
